...
However, I still missed a few of the niceties that I had been used to from `zsh` over the years - one of which was the [`extract` function provided in oh-my-zsh](https://github.com/robbyrussell/oh-my-zsh/blob/master/plugins/extract/extract.plugin.zsh).
Even after moving away from oh-my-zsh as my plugin manager in `zsh`, I had used my new plugin manager to grab [just that plugin](https://github.com/svanburen/dotfiles/blob/74dd7a02b83ca1874d721e242e0f466ca1f65692/zshrc#L13-L14) for my usage.
...
The function first checks if we have arguments - if we haven't, there's nothing to do, so we'll echo a usage string to stderr and exit.
```fish
...
Next, we handle if we've been passed a `-r` or `--remove` option as the first argument.
If we have, we'll remove the archive files after they've been successfully unarchived.
...
We first ensure that the filename arguments are valid files:
```fish
        if test ! -f $i
...
Lastly, we'll remove the original file if we've successfully unarchived the file, and end the loop and the function.
```fish
...
This was my first experience trying to port a larger function from `zsh` to `fish`, and it definitely took some playing around with the various test functions to get right.
